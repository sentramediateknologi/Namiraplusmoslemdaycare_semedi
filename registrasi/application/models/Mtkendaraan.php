<?php  
	...
	class MTkendaraan extends CI_Model
	{
		public function __construct() {
			parent::__construct();

	        ## declate table name here
	        $this->table_name = 'tr_peminjaman_kendaraan' ;
	        $this->id_perwakilan = 99 ;
	        $this->id_peminjam = $this->session->userdata['auth']->id ;
	        $this->role = $this->session->userdata['auth']->id_role ;

            $this->load->model('Memail', 'Email');
	    }

	    ## get all data in table
	    function getAll() {
	    	$this->db->select('
	    		tr_peminjaman_kendaraan.*, 
	    	...
	    	$this->db->join('data_user', 'data_user.id = tr_peminjaman_kendaraan.id_peminjam', 'left'); 
	    	$this->db->join('m_kendaraan', 'm_kendaraan.id = tr_peminjaman_kendaraan.id_kendaraan', 'left'); 


	    	$this->db->where('tr_peminjaman_kendaraan.is_active','1');

	    	$this->db->where_in('tr_peminjaman_kendaraan.status', array(1,2,3));

	    	if($this->role != 1) {
				$this->db->where('tr_peminjaman_kendaraan.id_peminjam',$this->id_peminjam);	    		
	    	}

            $this->db->order_by("tr_peminjaman_kendaraan.id", "desc");
	    ...
	    function getPengembalian() {
	    	...
	    	$this->db->where_in('tr_peminjaman_kendaraan.status', array(4,5));
	    ...
	    function getList() {
	    	...
	    	$this->db->select('tr_peminjaman_kendaraan.id, tr_peminjaman_kendaraan.name');
	    ...
	    function getByID($id) {
	        ...
	        $this->db->where(array('tr_peminjaman_kendaraan.id' => $id));
	    ...
	    public function getInAvailble()
	    {
	        ...
	        foreach ($result as $key => $row) {
	        	array_push($arr_result, $row->id_kendaraan);
	        }
