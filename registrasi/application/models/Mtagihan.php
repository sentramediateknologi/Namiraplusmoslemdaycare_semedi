<?php  
	...
	class MTagihan extends CI_Model
	{
		public function __construct() {
	        ...
	        $this->table_name = 'm_tagihan' ;
	    }

	    ## get all data in table
	    function getAll() {
            $this->db->select('m_tagihan.*, m_uraian.name as uraian');

	    	$this->db->where('m_tagihan.is_active','1');

            $this->db->order_by('id_uraian','asc');

            $this->db->join('m_uraian', 'm_uraian.id = m_tagihan.id_uraian', 'left'); 
